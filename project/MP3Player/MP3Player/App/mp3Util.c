...
void Mp3Stream(HANDLE hMp3, struct Node **curNode, 
               OS_EVENT * mbox, OS_FLAG_GRP *rxFlags)
{
    INT8U err;
    //INT8U *bufPos = pBuf;
    ...
    while (!done)
    {
        // Wait for STREAMING and DisplayTask
        OSFlagPend(rxFlags, 0x1, OS_FLAG_WAIT_CLR_ALL, 0, &err); // bits 1 and 2
        OSFlagPend(rxFlags, 0x2, OS_FLAG_WAIT_CLR_ALL, 0, &err); // bits 1 and 2
        
        // detect last chunk of pBuf
        Node *node = *(curNode);
        if ((node->data.size) - (node->data.pos) < MP3_DECODER_BUF_SIZE)
        {
            chunkLen = node->data.size - node->data.pos;
            done = OS_TRUE;
        }
                
        Write(hMp3, node->data.pStream, &chunkLen);
                
        node->data.pStream += chunkLen;
        node->data.pos += chunkLen;
        StreamProgress = (node->data.pos) * 100 / (node->data.size);
        OSMboxPost(mbox, (void *)&StreamProgress);
...
//        *pBuf += chunkLen;
//        *iBufPos += chunkLen;
//        StreamProgress = (*iBufPos) * 100 / (*bufLen);
//        OSMboxPost(mbox, (void *)&StreamProgress);
    }
