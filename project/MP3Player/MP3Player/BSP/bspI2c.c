...
void I2C1_init(void){
    ...
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_I2C1, ENABLE);   // RCC->APB1ENR |= RCC_APB1ENR_I2C1EN;     

    // enable clock for SCL and SDA pins
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB, ENABLE);  // RCC->AHB1ENR |= RCC_AHB1ENR_GPIOBEN;   
    ...
    //    typedef struct
    //    {
    //      uint32_t GPIO_Pin;              /* Set pins 8 and 9.                    @ref GPIO_pins_define */
    //      GPIOMode_TypeDef GPIO_Mode;     /* Set mode to alternate function (AF)  @ref GPIOMode_TypeDef */
    //      GPIOSpeed_TypeDef GPIO_Speed;   /* Set speed to 50 MHz                  @ref GPIOSpeed_TypeDef */
    //      GPIOOType_TypeDef GPIO_OType;   /* Set OType to open drain (OD)         @ref GPIOOType_TypeDef */
    //      GPIOPuPd_TypeDef GPIO_PuPd;     /* Set pull-up/pull-down to pull up     @ref GPIOPuPd_TypeDef */
    //    }GPIO_InitTypeDef        
    ...
    //    typedef struct
    //    {
    //      uint32_t I2C_ClockSpeed;          /* set clock speed to 100 kHz            @Specifies the clock frequency <= 400kHz */
    //      uint16_t I2C_Mode;                /* set to I2C mode                       @ref I2C_mode */
    //      uint16_t I2C_DutyCycle;           /* set to I2C_DutyCycle_2                @ref I2C_duty_cycle_in_fast_mode */
    //      uint16_t I2C_OwnAddress1;         /* set own address to 0                  @Specifies the first device own address; 7-bit or 10-bit address. */
    //      uint16_t I2C_Ack;                 /* set acknowledged address to 7 bits    @ref I2C_acknowledgement */
    //      uint16_t I2C_AcknowledgedAddress; /* @ref I2C_acknowledged_address */
    //    }I2C_InitTypeDef;
